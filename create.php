...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($idKaryawan_err) && empty($gambar_err) && empty($namaKaryawan_err) && empty($tanggalLahirKaryawan_err) && empty($idAsal_err) && empty($teleponKaryawan_err) && empty($tanggalMasukKaryawan_err) && empty($statusKaryawan_err) && empty($keterangan_err)){
        // Prepare an insert statement
        $sql = "INSERT INTO karyawan_adminitrasi (idKaryawan,gambar,namaKaryawan,tanggalLahirKaryawan,idAsal,teleponKaryawan,tanggalMasukKaryawan,statusKaryawan,keterangan) VALUES (?,?,?,?,?,?,?,?,?)";

        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sbsssssss", $param_idKaryawan, $param_gambar, $param_namaKaryawan, $param_tanggalLahirKaryawan, $param_idAsal, $param_teleponKaryawan, $param_tanggalMasukKaryawan, $param_statusKaryawan, $param_keterangan);
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        ...
                        <div class="form-group <?php echo (!empty($gambar_err)) ? 'has-error' : ''; ?>">
                                ...
                                <div class="col-sm-10">
                                    <input name="gambar" type="file" class="inputFile" />
                                    <span class="help-block"><?php echo $gambar_err;?></span>
                                </div>
                        </div>
                        <div class="form-group <?php echo (!empty($namaKaryawan_err)) ? 'has-error' : ''; ?>">
                                <div class="col-sm-2">
                                    <label>Nama Karyawan</label> 
                                </div>  
                                <div class="col-sm-10">
                                    <input type="text" nama="nama" class="form-control" value="<?php echo $namaKaryawan; ?>">
                                    <span class="help-block"><?php echo $namaKaryawan_err;?></span>
                                </div>
                        </div>
                        <div class="form-group <?php echo (!empty($tanggalLahirKaryawan_err)) ? 'has-error' : ''; ?>">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" tanggalLahirKaryawan="tanggalLahirKaryawan" class="form-control"><?php echo $tanggalLahirKaryawan; ?></span>
                                    <span class="help-block"><?php echo $tanggalLahirKaryawan_err;?></span>
                                </div>
                        </div>
                        <div class="form-group <?php echo (!empty($idAsal_err)) ? 'has-error' : ''; ?>">
                                <div class="col-sm-2">
                                    <label>id Asal Karyawan</label> 
                                </div>      
                                <div class="col-sm-10">
                                    <input type="text" idAsal="idAsal" class="form-select" value="<?php echo $idAsal; ?>"></span>
                                    <span class="help-block"><?php echo $idAsal_err;?></span>
                                </div>
                        </div>
                        <div class="form-group <?php echo (!empty($teleponKaryawan_err)) ? 'has-error' : ''; ?>">
                                <div class="col-sm-2">
                                    <label>Nomor Telepon Karyawan</label> 
                                </div>  
                                <div class="col-sm-10">
